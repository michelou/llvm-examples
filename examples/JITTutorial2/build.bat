...
set "_LLVM_OBJDUMP_CMD=%LLVM_HOME%\bin\llvm-objdump.exe"
...
@rem input parameter: %*
@rem output parameters: _CLEAN, _COMPILE, _RUN, _TEST, _TOOLSET, _VERBOSE
:args
set _CLEAN=0
set _COMPILE=0
set _DOC=0
...
goto :eof
set __LLVM_OBJDUMP_OPTS=-f -h

if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_LLVM_OBJDUMP_CMD%" %__LLVM_OBJDUMP_OPTS% "%__EXE_FILE%" 1>&2
) else if %_VERBOSE%==1 ( echo Dump PE/COFF infos for executable !__EXE_FILE:%_ROOT_DIR%=! 1>&2
)
call "%_LLVM_OBJDUMP_CMD%" %__LLVM_OBJDUMP_OPTS% "%__EXE_FILE%"
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to dump executable "%_EXE_NAME%" ^(ObjDump^) 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
