...
set "_LLVM_DIS_CMD=%LLVM_HOME%\bin\llvm-dis.exe"

if not exist "%GIT_HOME%\usr\bin\diff.exe" (
...
if %_TOOLSET%==1 ( set _TOOLSET_NAME=Clang/CMake
) else if %_TOOLSET%==2 (  set _TOOLSET_NAME=GCC/CMake
...
set __OPT_OPTS=-licm -o %_TARGET_DIR%\%_PROJ_NAME%.bc

if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_LLVM_OPT_CMD%" %__OPT_OPTS% "%__LL_FILE%" 1>&2
) else if %_VERBOSE%==1 ( echo Optimize IR code in file "%__LL_FILE%" 1>&2
)
call "%_LLVM_OPT_CMD%" %__OPT_OPTS% "%__LL_FILE%"
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to optimize IR code in file "%__LL_FILE%" 1>&2
    set _EXITCODE=1
    goto :eof
)
set __LLVM_DIS_OPTS=-o "%_TARGET_DIR%\%_PROJ_NAME%_opt.ll"
set "__BC_FILE=%_TARGET_DIR%\%_PROJ_NAME%.bc"

if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_LLVM_DIS_CMD%" %__LLVM_DIS_OPTS% %__BC_FILE% 1>&2
) else if %_VERBOSE%==1 ( echo Dissamble bitcode in file "%__BC_FILE%" 1>&2
)
call "%_LLVM_DIS_CMD%" %__LLVM_DIS_OPTS% %__BC_FILE%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to disassemble bitcode in file "%__BC_FILE%" 1>&2
    set _EXITCODE=1
    goto :eof
)
set _LL_FILE_OPT=%_TARGET_DIR%\%_PROJ_NAME%_opt.ll
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_DIFF_CMD%" "%__LL_FILE%" "%_LL_FILE_OPT%" 1>&2
) else if %_VERBOSE%==1 ( echo Compare IR code of file "!__LL_FILE:%_ROOT_DIR%=!" and "%_LL_FILE_OPT%" 1>&2
)
call "%_DIFF_CMD%" "%__LL_FILE%" "%_LL_FILE_OPT%"
goto :eof
